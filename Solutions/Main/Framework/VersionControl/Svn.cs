//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
// <copyright file="Svn.cs">(c) 2017 Mike Fourie and Contributors (http://www.MSBuildExtensionPack.com) under MIT License. See https://opensource.org/licenses/MIT </copyright>
// The code was written by Jozsef Fejes (http://joco.name).
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------
namespace MSBuild.ExtensionPack.VersionControl
{
    using System;
    ...
    using MSBuild.ExtensionPack.VersionControl.Subversion.Schema;
    ...
    public class Svn : BaseTask
    {
        ...
        protected override void InternalExecute()
        {
            ...
            if (SvnPath == null)
            {
                this.Log.LogError("A supported SVN client installation was not found");
                return;
            }
        ...
        private void ExecVersion()
        {
            ...
            if (this.Item == null)
            {
                this.Log.LogError("The Item parameter is required");
                return;
            }
            ...
            var output = Utilities.ExecuteWithLogging(this.Log, Path.Combine(SvnPath, SvnVersionExecutableName), cmd.ToString(), false);
            ...
            if (!this.Log.HasLoggedErrors)
            {
                ...
                if (!m.Success)
                {
                    ...
                    this.Log.LogError("Version: Invalid output from SVN tool: no regex  match. Svn tool output was: {0}", output);
                    return;
                }
        ...
        private void ExecInfo()
        {
            ...
            cmd.AppendFileNameIfNotNull(this.Item);
            var output = Utilities.ExecuteWithLogging(this.Log, Path.Combine(SvnPath, SvnExecutableName), cmd.ToString(), false);

            if (!this.Log.HasLoggedErrors)
            {
                // deserialize the response
                var xs = new XmlSerializer(typeof(MSBuild.ExtensionPack.VersionControl.Subversion.Schema.infoType));
                infoType info;
                try
                {
                    using (var sr = new StringReader(output))
                    {
                        info = (MSBuild.ExtensionPack.VersionControl.Subversion.Schema.infoType)xs.Deserialize(sr);
                    }
                }
                catch (InvalidOperationException invalidOperationException)
                {
                    // not versioned or really an error, we don't care
                    this.Log.LogError("Info: Invalid output from SVN tool: InvalidOperationException: {0}", invalidOperationException.Message);
                    return;
                }
                ...
                if (info.entry == null || info.entry.Length != 1)
                {
                    ...
                    this.Log.LogError("Info: Invalid output from SVN tool: Svn info type entry was null or had a length different from 1");
                    return;
                }
        ...
        private void ExecGetProperty()
        {
            ...
            {
                this.Log.LogError("The Item and PropertyName parameters are required");
                return;
            }
            ...
            {
                // deserialize the response
                var xs = new XmlSerializer(typeof(MSBuild.ExtensionPack.VersionControl.Subversion.Schema.propertiesType));
                propertiesType props;
                try
                {
                    using (var sr = new StringReader(output))
                    {
                        props = (MSBuild.ExtensionPack.VersionControl.Subversion.Schema.propertiesType)xs.Deserialize(sr);
                    }
                }
                catch (InvalidOperationException ioe)
                {
                    this.Log.LogError(string.Format(CultureInfo.InvariantCulture, "InvalidOperationException: Invalid output from SVN tool - {0}", ioe));
                    return;
                }
                ...
                if (props.target == null || (props.target.Length == 1 && props.target[0].property == null))
                {
                    ...
                    this.Log.LogWarning("The SVN property doesn't exist");
                    return;
                }
                ...
                {
                    // this really shouldn't happen
                    this.Log.LogError("Invalid output from SVN tool. Error Condition = props.target.Length != 1");
                    return;
                }
                ...
                {
                    // this really shouldn't happen
                    this.Log.LogError("Invalid output from SVN tool. Error Condition = props.target[0].property.Length != 1");
                    return;
                }
                ...
                {
                    // this really shouldn't happen
                    this.Log.LogError(string.Format(CultureInfo.InvariantCulture, "Invalid output from SVN tool. Error Condition = props.target[0].property[0].name != this.PropertyName. Where this.PropertyName = {0}", this.PropertyName));
                    return;
                }
        ...
        private void ExecCheckout()
        {
            ...
            {
                this.Log.LogError("The Items and Destination parameters are required");
                return;
            }
        ...
        private void ExecUpdate()
        {
            ...
            {
                this.Log.LogError("The Items parameter is required");
                return;
            }
        ...
        private void ExecCopy()
        {
            ...
            {
                this.Log.LogError("There appears to be quotes in the commit message. This is not supported yet.");
                return;
            }
        ...
        private void ExecExport()
        {
            ...
            {
                this.Log.LogError("The Item and Destination parameters are required");
                return;
            }
